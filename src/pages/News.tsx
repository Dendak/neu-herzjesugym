...
export default function News() {
  ...
  return (
    <>
      <PageHeader eyebrow={cat ? 'Kategorie' : 'Aus dem Schulleben'} title={cat ? `${cat.name}.` : 'Aktuelles.'}
        lead={cat ? `${cat.count} Beiträge aus dem Bereich ${cat.name}.` : 'Berichte, Projekte, Veranstaltungen und Neuigkeiten aus unserer Schule.'} />
      <div className="wrap pb-16">
        ...
        {(posts.loading || !posts.data) && !posts.error && (
          <div className="grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 6 }).map((_, i) => <SkeletonCard key={i} />)}</div>
        )}
